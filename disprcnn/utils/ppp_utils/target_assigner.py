...
from disprcnn.utils.ppp_utils.anchor_generator_builder import build_anchor_generator
from disprcnn.utils.ppp_utils.similarity_calculator import build_similarity_calculator
...
class TargetAssigner:
    ...
    def assign(self,
        ...
        if anchors_mask is not None:
            if isinstance(anchors_mask, np.ndarray):
                prune_anchor_fn = lambda _: np.where(anchors_mask)[0]
            else:
                prune_anchor_fn = lambda _: torch.where(anchors_mask)[0]
        else:
        ...
        return self.create_target_np(anchors,
                                     gt_boxes,
                                     similarity_fn,
                                     box_encoding_fn,
...
def build_target_assigner(target_assigner_config, box_coder):
