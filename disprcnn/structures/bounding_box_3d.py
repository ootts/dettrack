...
class Box3DList(object):
    ...
    def __init__(self, bbox_3d, mode="corners", frame="rect"):
    ...
    def convert(self, mode):
        ...
        if mode == "corners":
            ...
            ry = -(torch.atan2(dif[:, 2], dif[:, 0])).view(-1, 1)
            xyz = ((corners[7] + corners[0]) / 2).view(-1, 3)
            alpha = ry + torch.atan2(xyz[:, 2].view(-1, 1), xyz[:, 0].view(-1, 1)) - np.pi / 2
            l = torch.norm((corners[0] - corners[3]), dim=1).view(-1, 1)
            h = torch.norm((corners[0] - corners[1]), dim=1).view(-1, 1)
            w = torch.norm((corners[0] - corners[4]), dim=1).view(-1, 1)
            bbox_3d = torch.cat((alpha, l, h, w, xyz), dim=-1)
    ...
    def _split_into_corners(self):
        ...
        elif self.mode == "alpha_lhwxyz":
            alpha, l, h, w, x, y, z = self.bbox_3d.split(1, dim=-1)
            ry = alpha + (- torch.atan2(x, z))
            # TODO make code simple
            zero_col = torch.zeros(ry.shape).to(self.device)
            ones_col = torch.ones(ry.shape).to(self.device)
    ...
    def filter_out_point_cloud(self, pointcloud):
        """
        filter pointcloud
        ...
        keeps = torch.zeros(pointcloud.shape[0]).bool()
