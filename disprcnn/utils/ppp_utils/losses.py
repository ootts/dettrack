    ...
    def __call__(self,
        ...
        if ignore_nan_targets:
            target_tensor = torch.where(torch.isnan(target_tensor),
                                        prediction_tensor,
                                        target_tensor)
        return self.compute_loss(prediction_tensor, target_tensor, **params)

    @abstractmethod
    def compute_loss(self, prediction_tensor, target_tensor, **params):
...
class WeightedL2LocalizationLoss(Loss):
    ...
    def __init__(self, code_weights=None):
        ...
        if code_weights is not None:
            ...
            self.code_weights = Variable(torch.from_numpy(self.code_weights).cuda())
        else:
    ...
    def compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        if self.code_weights is not None:
            self.code_weights = self.code_weights.type_as(prediction_tensor)
            self.code_weights = self.code_weights.view(1, 1, -1)
            diff = self.code_weights * diff
...
class WeightedSmoothL1LocalizationLoss(Loss):
    ...
    def compute_loss(self, prediction_tensor, target_tensor, weights=None):
        """Compute loss function.
        ...
        diff = prediction_tensor - target_tensor
        if self.code_weights is not None:
            code_weights = self.code_weights.type_as(prediction_tensor)
            diff = code_weights.view(1, 1, -1) * diff
        abs_diff = torch.abs(diff)
        abs_diff_lt_1 = torch.le(abs_diff, 1 / (self.sigma ** 2)).type_as(abs_diff)
        loss = abs_diff_lt_1 * 0.5 * torch.pow(abs_diff * self.sigma, 2) \
...
class SigmoidFocalClassificationLoss(Loss):
    ...
    def compute_loss(self,
        ...
        if self.alpha is not None:
            alpha_weight_factor = (target_tensor * self.alpha +
                                   (1 - target_tensor) * (1 - self.alpha))
...
class SoftmaxFocalClassificationLoss(Loss):
    ...
    def compute_loss(self,
        ...
        if class_indices is not None:
            weights *= indices_to_dense_vector(class_indices,
                                               prediction_tensor.shape[2]).view(1, 1, -1).type_as(prediction_tensor)
        per_entry_cross_ent = (softmax_cross_entropy_with_logits(
            labels=target_tensor, logits=prediction_tensor))
        ...
        if self.alpha is not None:
            alpha_weight_factor = torch.where(target_tensor[..., 0] == 1,
                                              torch.tensor(1 - self.alpha).type_as(per_entry_cross_ent),
                                              torch.tensor(self.alpha).type_as(per_entry_cross_ent))
        focal_cross_entropy_loss = (modulating_factor * alpha_weight_factor *
                                    per_entry_cross_ent)
        return focal_cross_entropy_loss * weights
    ...
    def compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        if self.bootstrap_type == 'soft':
            bootstrap_target_tensor = self.alpha * target_tensor + (
                    1.0 - self.alpha) * torch.sigmoid(prediction_tensor)
        else:
            bootstrap_target_tensor = self.alpha * target_tensor + (
                    1.0 - self.alpha) * (torch.sigmoid(prediction_tensor) > 0.5).float()
