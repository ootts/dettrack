...
class IDispnetPart2PosOnnx(nn.Module):
    def __init__(self, model):
        super(IDispnetPart2PosOnnx, self).__init__()
    ...
    def forward(self, left, right):
        ...
        :return:
        """
        return self.model.forward_onnx_part2_pos(left, right)
...
def main():
    ...
    model = trainer.model
    model = IDispnetPart2PosOnnx(model)
    model.eval()
    model.cuda()
    ...
    output_onnx = osp.join(cfg.trt.onnx_path, "idispnet_part2_pos.onnx")
    ...
    print('to engine')
    engine_path = osp.join(cfg.trt.convert_to_trt.output_path, "idispnet_part2_pos.engine")
