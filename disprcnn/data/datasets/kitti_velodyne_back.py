...
#         # if not osp.exists(info['velodyne_path']):
#         #     v_path = pathlib.Path(root_path) / info['velodyne_path']
#         # else:
#         #     v_path = pathlib.Path(info['velodyne_path'])
#         # v_path = v_path.parent.parent / (v_path.parent.stem + "_reduced") / v_path.name
#         # pointpillars/kitti_second/training/velodyne_reduced/00
#         split = "training" if self.split in ['train', 'val'] else "testing"
#         v_path = osp.join(root_path, split, 'velodyne_reduced', f"{idx:06d}.bin")
#         points = np.fromfile(v_path, dtype=np.float32).reshape([-1, self.num_point_features])  # load points
...
#             ##############  ↓ Step 2: noise, random Rt,scaling, flip  ↓  ##############
...
#         ##############  ↓ Step 3: make gt for each anchor  ↓  ##############
#         # [0, -40, -3, 70.4, 40, 1]
...
#             points[:, 2] < pc_range[5],
#         ])
#         vis3d.add_point_cloud(points[:, :3][keep], name='points_keep')
#         example = {
#             'voxels': voxels,
...
#             'P2': P2,
#         }
#         vis3d.add_point_cloud(coordinates * voxel_size[::-1][None], name='coordinates')
...
#             coors = coordinates
#             dense_voxel_map = sparse_sum_for_anchors_mask(coors, tuple(grid_size[::-1][1:]))
#             dense_voxel_map = dense_voxel_map.cumsum(0)
#             dense_voxel_map = dense_voxel_map.cumsum(1)
#             anchors_area = fused_get_anchors_area(dense_voxel_map, anchors_bv, voxel_size, pc_range, grid_size)
